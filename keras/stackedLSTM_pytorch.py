"""
Stacked LSTM (多层LSTM堆叠) 模型在 IMDB 情感分析任务上的 PyTorch 实现

此文件是 stackedLSTM.py 的 PyTorch 版本，实现相同的功能。
参考 LSTM_pytorch.py 的架构，实现三层 LSTM 堆叠。
参考 SimpleRNN_pytorch.py 的本地数据集加载和训练曲线可视化功能。
...
模型架构说明：
Embedding → LSTM1 (return_sequences=True) → LSTM2 (return_sequences=True) → LSTM3 (return_sequences=False) → Dense(sigmoid)

文件结构：
    1. 导入必要的 PyTorch 库
    2. 定义超参数
    3. 自定义 IMDB Dataset 类和辅助函数
    4. 构建 Stacked LSTM 模型类
    5. 训练和评估函数
    6. 训练曲线可视化函数
    7. 主程序执行流程
"""
...
class StackedLSTMModel(nn.Module):
    """
    三层堆叠 LSTM 模型，与 Keras 版本功能相同

    模型架构：
    Embedding → LSTM1 (return_sequences=True) → Dropout →
    LSTM2 (return_sequences=True) → Dropout →
    LSTM3 (return_sequences=False) → Dense(sigmoid)
    ...
    def forward(self, x):
        ...
        # 第一层 LSTM: (batch, seq_len, embedding_dim) → (batch, seq_len, hidden_dim)
        # 输出所有时间步的隐藏状态
        ...
        # 第二层 LSTM: (batch, seq_len, hidden_dim) → (batch, seq_len, hidden_dim)
        # 输出所有时间步的隐藏状态
        ...
        # 第三层 LSTM: (batch, seq_len, hidden_dim) →
        # lstm3_out: (batch, seq_len, hidden_dim) - 所有时间步的隐藏状态
        # (hidden_state, cell_state): 最后时间步的隐藏状态和细胞状态
        lstm3_out, (hidden_state, cell_state) = self.lstm3(lstm2_out)

        # 只取第三层最后一个时间步的隐藏状态
        # hidden_state 的形状: (1, batch, hidden_dim) [因为 num_layers=1]
        last_hidden_state = hidden_state[-1]  # 形状: (batch, hidden_dim)

        # 第三层 dropout（应用于最后一个时间步的隐藏状态）
        if self.dropout3 is not None:
            last_hidden_state = self.dropout3(last_hidden_state)
...
def plot_training_history(history, test_loss=None, test_acc=None):
    ...
    # 设置标题
    ax1.set_title('Stacked LSTM Training Curves', fontsize=14, fontweight='bold')
    ...
    # 保存图像
    plt.savefig('stacked_lstm_training_curves.png', dpi=300, bbox_inches='tight')
    print("Training curves saved to 'stacked_lstm_training_curves.png'")
...
def main():
    ...
    # 7. 可选：保存模型
    # torch.save(trained_model.state_dict(), 'stacked_lstm_model.pth')
